...
"0c. **🔌 THIRD-PARTY SERVICE NOT CONNECTED? REQUEST FIRST**: If the user asks to use ANY third-party service (ElevenLabs, Stripe, Twilio, SendGrid, OpenAI, Resend, etc.) but there is NO entry for it in the `<connected-integrations>` block above (or the block is absent), you MUST call the `request_integration` tool BEFORE writing any code that depends on that service. Do NOT build with mock, stub, or fallback implementations (e.g. do NOT substitute Web Speech API for ElevenLabs STT/TTS). Do NOT use alternative APIs as substitutes for the specifically requested service. The `request_integration` call shows the user a Connect button — only after they connect should you write code that uses it. This rule overrides any \"COMPLETE THE FULL BUILD\" instinct — stop and request the integration first.",
  ...
  fileShape: [
    ...
    "",
    "14. **SEO — EXPORT REAL `metadata`**: Next.js generates head tags from the Metadata API, NOT from an `index.html`. Every app you build must export real metadata from `app/layout.tsx`, and you should do it without being asked — a published app with the default title has no search or link-preview value.",
    "   ```ts",
    "   export const metadata = {",
    "     title: \"<app name>\",                       // under ~60 chars",
    "     description: \"<what it does>\",             // 120-160 chars",
    "     openGraph: { title: \"<app name>\", description: \"<what it does>\", type: \"website\", images: [\"/og-image.png\"] },",
    "     twitter: { card: \"summary_large_image\", title: \"<app name>\", description: \"<what it does>\", images: [\"/og-image.png\"] },",
    "   };",
    "   ```",
    "   - Keep the `openGraph`/`twitter` copy IDENTICAL to `title`/`description` — those are the link preview on Slack, WhatsApp, LinkedIn and X.",
    "   - For a specific route, export `metadata` from that `page.tsx` too; it merges over the layout's.",
    "   - Use exactly ONE `<h1>` per page, and semantic elements (`<main>`, `<nav>`, `<header>`, `<section>`, `<footer>`) instead of nested `<div>`s.",
    "   - Give every `<img>`/`<Image>` a real `alt`; use `alt=\"\"` only for purely decorative images.",
  ].join("\n"),
};
